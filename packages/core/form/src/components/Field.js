  ...
  if (type === 'checkbox') {
    if(!isEmpty(props.elements)){
      field= props.elements.map((item,index)=>{
        return [...field,<Checkbox {...props} label={item.label} checked={item.initValue} radio={props.details ==='radio' ? true : false } toggle={props.details==='toggle' ? true : false } key={index}/>]
      })
    }else{
      field=<Checkbox {...props} radio={props.details ==='radio' ? true : false } toggle={props.details==='toggle' ? true : false } label={label} />
    }
